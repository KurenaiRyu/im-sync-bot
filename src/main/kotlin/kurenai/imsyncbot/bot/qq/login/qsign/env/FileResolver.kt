...
import kurenai.imsyncbot.utils.parseHex
...
class FileResolver(
    ...
    private fun resolve(
        emulator: Emulator<AndroidFileIO>,
        path: String,
        oflags: Int,
        def: FileResult<AndroidFileIO>?
        ...
        if (path == "/system/bin/sh" || path == "/system/bin/ls" || path == "/system/lib/libc.so") {
            return FileResult.success(
                ByteArrayFileIO(
                    oflags,
                    path,
                    byteArrayOf(0x7f, 0x45, 0x4c, 0x46, 0x02, 0x01, 0x01, 0x00)
                )
            )
        }
        ...
        if (path.contains("system_android_l2") || path.contains("android_lq")) {
            ...
            if (!file.exists()) {
                file.writeBytes("619F9042CA821CF91DFAF172D464FFC7A6CB8E024CC053F7438429FA38E86854471D6B0A9DE4C39BF02DC18C0CC54A715C9210E8A32B284366849CBB7F88C634AA".parseHex())
            }
